...
1. **Your primary AI phone app is required.** **ChatGPT, Claude, and Cursor users:** use your provider's official mobile app and sign in with the same account you use on your computer. If you have a subscription, make sure it is the account that holds it. Cursor users whose phone does not support the native app can use [Cursor in the browser](https://cursor.com/agents).
   ...
   **Google users:** use the [Gemini phone app](https://support.google.com/gemini/answer/14554984) for chat and its available agent features. Sign in with the Google account you use for your Google AI subscription, if you have one. Antigravity is the separate computer harness. To reach an already configured Antigravity instance from your phone, open [Antigravity Remote Control](https://antigravity.google.com/) in your browser, sign in with the same Google account as that instance, and select your computer. Gemini chat does not connect you to that instance. Handle Antigravity during Phase 2 Computer Setup; you can use a different companion for Phase 1.

   **Gemini and Spark compatibility:** neither could read this guide in my latest phone tests, even after our first access fixes. Claude read it without trouble. One final text-only route is being checked; Gemini and Spark are not yet verified companions for this pack. This concerns reading the guide, not their ability to discuss an idea or generate files.

   **If your Google chat cannot load the guide, use Claude for Phase 1.** My baseline is **Sonnet 4.6 with Medium effort**; the free allowance has been enough in my testing. Select those settings when your app offers them. Available models and allowances can vary; you do not need to buy an upgrade for this pack. Preserve your progress when switching, and use Antigravity later if that is your preferred computer harness.
...
- **Gemini:** use it for this route only when it can read the required instructions; use Claude for Phase 1 if guide access fails. For agent-driven deployment, continue in Antigravity on your computer or through your existing Antigravity browser connection. For a static file upload, use the Cloudflare iPhone guide with files from Gemini or any other agent.
