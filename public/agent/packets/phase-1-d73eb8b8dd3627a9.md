...
If you are Gemini responding in Gemini chat, stop here and show the learner https://starter.devthomas.site/recommendations; in that case, do not retry fetching or continue with the instruction packet. Gemini chat is not supported for this pack. Otherwise, continue below. This restriction does not apply to the separate Antigravity computer harness or Google AI Studio builder.
...
## Phase 1 two-build challenge

Help the learner clarify their idea in this conversation; they run the build prompt in the instant builder. At the instant-build step, offer a first-time learner who has neither a reviewed prompt nor a useful first build this optional experiment: "Make one baseline build from your raw idea, then come back and tell me what you actually meant. I'll help you turn that into a reviewed build prompt so you can make a second version and compare them."

If accepted, guide one step at a time:

1. Preserve the learner's original wording verbatim as the Build A prompt. Give the learner that wording to run in their chosen builder without improving it, adding requirements, or making it intentionally worse. Have them return after trying the result's main action; do not rescue or coach improvements to Build A before the comparison.
2. In this conversation, ask a compact refinement round about the intended audience, main action, inputs or content, exclusions, visual intent, and what working means. Reuse known answers and ask only about consequential missing decisions; do not repeat stack selection or run a long requirements interview.
3. Synthesize a concise Build B prompt/spec from those decisions. Make assumptions visible for learner review. Wait for their approval or corrections before sending them back to the builder; resolve corrections in the prompt first.
4. Have them run the reviewed prompt in the same chosen builder for Build B, keeping the same idea and the same short time window for each build so the prompt/context is the comparison's focus. Preserve useful work; do not require two deployments, saved repositories, or permanent projects.
5. Ask them to try the same intended main action in both versions: what changed, which hidden assumptions were resolved, and does Build B better match what they meant? Connect observed differences to the decisions/context added. Do not promise Build B is better or teach that a longer prompt is better.
6. Let the learner choose the version to keep, then continue normal testing and iteration until its main action works.

If the learner declines, already has a strong prompt, has completed an instant build, or is resuming progress, skip the experiment and reuse prior work. Follow the normal refinement-first path for any missing work; do not rebuild or overwrite a useful app just to stage a comparison.

When useful, keep a short nonsecret note in the existing `instant-build` step's `notes`: accepted, declined, skipped for prior work, or completed; whether the refined prompt was reviewed; and the learner's comparison (identify a companion summary as such). Preserve the raw idea there only when the learner wants it saved. These notes are optional teaching evidence, never graduation evidence. Do not add a required step, schema field, or completion gate; the existing canonical Phase 1 outcomes alone decide graduation.
...
Pick a tiny idea with an observable result: a dinner shuffler, a score counter, or a list you can filter. Start in your companion chat.

First build? Your companion will offer the optional [Two-Build Challenge](#optional-the-two-build-challenge) before refining your idea. Otherwise, follow this path:

**Idea → chat refinement → reviewed prompt → instant builder → test → iterate.**

1. Tell your companion what you want to make. Together, clarify who it is for, the main action, the inputs or content, and what the result should be. Include important exclusions and any visual direction that matters to you.
2. Have it turn those decisions into a concise build prompt, with assumptions you can check and one action that would prove it worked.
3. Review and correct the prompt. Then paste the approved version into your chosen builder in the browser.

Chat is where you work out what you mean; the builder is where you try making it. A useful prompt resolves decisions and shares context. Adding words alone does not make it better.

### Optional: the Two-Build Challenge

Want to feel the difference for yourself? Before refining your idea, ask your companion to guide this small experiment:

1. **Build A:** paste your raw one-line idea into the builder unchanged. Try the result once; leave improvements for the comparison.
2. **Back to chat:** explain what you actually imagined. Refine it together using the questions above, then review and correct the build prompt.
3. **Build B:** use that reviewed prompt in a fresh attempt with the same builder. Keep the same idea and give each build the same short time window.
4. **Compare:** try the same main action in both. What works, what is missing, which assumptions changed, and which interface comes closer to what you meant? Choose the version you want to keep, then continue testing and iterating below.

This is an experiment, not a test you can fail. Build B does not have to win. You do not need two saved projects, repositories, or deployments. Already have a reviewed prompt or a useful build, or returning to saved progress? Keep that work and skip the comparison. Declining it changes nothing about Phase 1 completion.

Here is how that refinement looked in one of my projects:
...
I already knew the idea. The experiment was seeing how far a clear request could take it in Google AI Studio. I first asked an AI assistant to turn my rough description into a build prompt. Then I reviewed what it had added before giving the prompt to the builder. That is the same chat, review, then build path you can use here. Resolving the assumptions mattered more than the prompt's length.
