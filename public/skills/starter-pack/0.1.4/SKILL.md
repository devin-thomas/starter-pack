...
Offer deployment only after the learner has a working instant app, is confident in the idea, is comfortable with a public app, and has a usable route: a computer now, an already configured remote harness capable of project work, or a supported cloud coding environment. Ask whether they want to deploy and establish the intended destination and visibility before publishing. Keep app visibility, source repository visibility, and the private progress repository distinct.

Follow the current Phase 1 provider guidance. Claude Code cloud and Cursor Cloud Agents can work without the learner's computer, but require repository access and hosting configuration. Ordinary GitHub context access is not write access. Do not promise native ChatGPT mobile deployment through the read-only GitHub app or assume web MCP tools are present on the phone. For this pack, route Gemini/Spark users to Antigravity for deployment. Check actual capabilities before taking action; never infer them from the provider name.

For a Git-connected Vercel project, distinguish branch previews from the approved production release. Confirm the deployed URL and have the learner try its main action. Record the chosen route, consent, live link, and reported or checked result as nonsecret notes under `phase-1-deployment-extra-credit`. Declined extra credit is skipped; accepted work postponed for access or setup is deferred with a resume point. Neither status prevents Phase 1 completion. Do not force Computer Setup early or count this deployment as completion of Phase 2.
